...
// NewRefreshableDialer constructs a new ContextDialer backed by a refreshable DialerParams.
// The dialer is reconstructed when any of the parameters are updated.
func NewRefreshableDialer(ctx context.Context, r refreshable.Refreshable[DialerParams]) ContextDialer {
	...
	dialer, _ := refreshable.Map(r, func(p DialerParams) ContextDialer {
		...
		return newDialer(ctx, p)
	})
	return &RefreshableDialer{Refreshable: dialer}
}

func newDialer(ctx context.Context, p DialerParams) ContextDialer {
	dialer := &net.Dialer{
