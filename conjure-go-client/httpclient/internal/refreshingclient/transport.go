...
func NewRefreshableTransport(ctx context.Context, pArg refreshable.Refreshable[TransportParams], refreshableConfig refreshable.Validated[*tls.Config], dialer ContextDialer) http.RoundTripper {
	p, _, _ := refreshable.Validate(ctx, pArg, func(ctx context.Context, params TransportParams) error {
		return nil
	})
	mapped, _ := refreshable.MergeValidated(p, refreshableConfig, func(p TransportParams, t *tls.Config) *http.Transport {
