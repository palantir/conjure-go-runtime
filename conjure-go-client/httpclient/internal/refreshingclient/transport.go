...
func NewRefreshableTransport(ctx context.Context, p refreshable.Refreshable[TransportParams], tlsConfig *tls.Config, dialer ContextDialer) http.RoundTripper {
	transport, _ := refreshable.Map(p, func(params TransportParams) *http.Transport {
		return newTransport(ctx, params, tlsConfig, dialer)
	})
