...
func newClient(ctx context.Context, b *clientBuilder, params ...ClientParam) (Client, error) {
	...
	if b.URIs == nil {
		return nil, werror.ErrorWithContextParams(ctx, "httpclient URLs must be set in configuration or by constructor param", werror.SafeParam("serviceName", b.HTTP.ServiceNameTag.Value()))
	}
	if !b.AllowEmptyURIs && len(b.URIs.CurrentStringSlice()) == 0 {
		return nil, werror.WrapWithContextParams(ctx, ErrEmptyURIs, "", werror.SafeParam("serviceName", b.HTTP.ServiceNameTag.Value()))
	}
