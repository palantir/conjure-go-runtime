...
func TestRefreshableClientConfig(t *testing.T) {
	...
	testDefaultClient := func(t *testing.T, client *clientImpl) {
		...
		if client.backoffOptions != nil {
			assert.Nil(t, client.backoffOptions.Current().MaxAttempts)
		}
	...
	refreshableServicesConfig, _ := refreshable.Map[[]byte, ServicesConfig](refreshableConfigBytes, func(i []byte) ServicesConfig {
		var c ServicesConfig
		if err := yaml.Unmarshal(i, &c); err != nil {
			panic(err)
		}
		return c
	})
	...
	currentHTTPClient := client.(*clientImpl).client.Current
