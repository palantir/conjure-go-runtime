...
func (b *bodyMiddleware) setRequestBody(req *http.Request) (func(), error) {
	cleanup := func() {} // default no-op cleanup function
	...
	if b.requestEncoder != nil {
		if b.bufferPool != nil {
			...
			requestBody = RequestBodyInMemory(buf)
		} else {
			...
			requestBody = RequestBodyInMemory(bytes.NewReader(buf))
		}
	} else {
		...
		binaryBody, ok := b.requestInput.(RequestBody)
		if !ok {
